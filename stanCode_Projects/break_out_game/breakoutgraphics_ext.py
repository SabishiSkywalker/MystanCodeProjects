...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH, paddle_height=PADDLE_HEIGHT,
        ...
        # Create scoreboard and initialize the score
        self._scoreboard_label = GLabel('Score: 0', x=(window_width-paddle_width)//2, y=40)
        self._scoreboard_label.font = '-30'
        self.__score = 0
        self.window.add(self._scoreboard_label)

        # Initialize the number of lives
        self.__lives = 3

        # Create lives label
        self.__lives_label = GLabel(f"Lives: {self.__lives}")
        self.__lives_label.font = "Helvetica-18-bold"
        self.__lives_label.color = 'Black'
        self.__lives_label.x = self.window.width - 100
        self.__lives_label.y = self.window.height - 20
        self.window.add(self.__lives_label)
    ...
    def move_paddle(self, mouse):
        ...
        if self._paddle.x < 0:
            self._paddle.x = 0
        elif self._paddle.x + self._paddle.width > self.window.width:
            self._paddle.x = self.window.width - self._paddle.width
        else:
            self._paddle.x = mouse.x - self._paddle.width//2
            # Keep paddle at a fixed y-coordinate above the window bottom
            self._paddle.y = self.window.height - self._paddle_offset - self._paddle.height
    ...
    def handle_collision(self):
        self.collided = False
        # Create a list to hold colliding bricks
        colliding_bricks = []
        ...
        for x in range(int(self.ball.x), int(self.ball.x + self.ball.width + 1), self.ball.width):
            for y in range(int(self.ball.y), int(self.ball.y + self.ball.height + 1), self.ball.height):
                ...
                if obj is not None and obj is not self.__lives_label:
        ...
        for brick in colliding_bricks:
            ...
            # Update the score
            self.__score += 1
            self._scoreboard_label.text = 'Score: ' + str(self.__score)
            # Reverse the y direction of the ball
            self.__dy = -self.__dy
            self.collided = True
            break
        # Reset the collided flag if there was no collision
        if not self.collided:
            self.collided = True
        # Check for collision with bottom edge of window
        if self.ball.y + self.ball.height >= self.window.height:
            self.__lives -= 1
            self.__lives_label.text = f'Lives: {self.__lives}'
